...
def test_download_job(tmp_path_cd: Path, feedobj_lautsprecher: dict[str, Any]) -> None:
    ...
    job = download.DownloadJob(episode=episode, target=Path("file.tmp"), feed_info=feed.feed, progress=mock_progress)
    result = job()
...
def test_download_already_exists(tmp_path_cd: Path, feedobj_lautsprecher_notconsumed: dict[str, Any]) -> None:
    ...
    job = download.DownloadJob(episode=episode, target=Path("file.tmp"), feed_info=feed.feed)
